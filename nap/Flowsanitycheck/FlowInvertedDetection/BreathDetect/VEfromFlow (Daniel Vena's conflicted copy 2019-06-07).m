function [time,Vflow,BB_i_start,BB_i_mid,BB_i_end,BB_t,VI,VE,Ttot,leak,...
    IEratio,VT,Vpeak,Vpeakmean,Apnea_B,Vflow_out,VTi,VTe] = VEfromFlow(time,Vflow)
...
if settings.sqrt_scaling
    if 0
        ...
        [Vflow,leak,IEratio]=sqrtscaling(time,Vflow,settings.plotfiguresqrtscaling);
        toc
        tic
        leak=leak+leak1;
        if 0
...
for loopnumber=1:2 %second loop subtracts extreme leaks
    ...
    if settings.modBB_i_start&&length(BB_i_start)>5
        ...
        for i=1:Nwindows %this loop is slow
            ...
            ri = li+slidewi;
            if ri>length(Vflow),ri=length(Vflow); end
            Vflow_delta(i)=std(Vflow2(li:ri));%
            Vflow_deltaB(i)=std(detrend(Vflow2(li:ri)));%
%             Vflow_delta(i)=(max(Vflow2(li:ri))-min(Vflow2(li:ri)))/(2*sqrt(2));
            Vflow_median(i)=median(Vflow2(li:ri));
            temptime(i) = median(time(li:ri));
